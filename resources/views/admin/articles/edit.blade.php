...
@section('content')
    ...
    <form method="post" action="{{action('ArticlesController@update', $article->id)}}">
        ...
        @method('PUT')
        Название статьи:<br>
        <input type="text" class="form-control" placeholder="Заголовок" name="title" value="{{$article->title}}"><br>
        Текст статьи:<br>
        <textarea class="form-control" style="height: 150px" name="body">{{$article->body}}</textarea><br>
